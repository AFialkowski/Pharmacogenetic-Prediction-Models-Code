###############################################################################
## Code to summarize model results with Logistic error terms
###############################################################################
...
# summary of outcomes
y1 <- read.table(paste(f2, "Sum_y_betas1.txt", sep = ""), header = T, sep = "\t", quote = "")
y2 <- read.table(paste(f2, "Sum_y_betas2.txt", sep = ""), header = T, sep = "\t", quote = "")
y3 <- read.table(paste(f2, "Sum_y_betas3.txt", sep = ""), header = T, sep = "\t", quote = "")

y <- rbind(y1, y2, y3)
print(xtable(y, align = c("l", rep("c", 9)), digits = c(rep(0, 4), rep(3, 6))), 
             include.rownames = F, include.colnames = T)

# computation times: minutes per 1000 models
# treatment group:
...
for (i in 1:length(tx)) {
  for (k in 1:3) {
    tEN25 <- c(tEN25, readRDS(paste(f2, "Time_EN1_cv_", tx[i], "_a25_", n_tx[i], "_betas", k, ".rds", sep = "")))
    tEN50 <- c(tEN50, readRDS(paste(f2, "Time_EN1_cv_", tx[i], "_a50_", n_tx[i], "_betas", k, ".rds", sep = "")))
    tEN75 <- c(tEN75, readRDS(paste(f2, "Time_EN1_cv_", tx[i], "_a75_", n_tx[i], "_betas", k, ".rds", sep = "")))
    tL <- c(tL, readRDS(paste(f2, "Time_EN1_cv_", tx[i], "_", n_tx[i], "_betas", k, ".rds", sep = "")))
    tBL <- c(tBL, readRDS(paste(f2, "Time_Blasso1_", tx[i], "_", n_tx[i], "_betas", k, ".rds", sep = "")))
    tM <- c(tM, readRDS(paste(f2, "Time_Earth1_cv_", tx[i], "_", n_tx[i], "_betas", k, ".rds", sep = "")))
...
# 2 tx groups
terr1 <- read.table(paste(f2, "tx2_terr_betas_1.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr2 <- read.table(paste(f2, "tx2_terr_betas_2.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr3 <- read.table(paste(f2, "tx2_terr_betas_3.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
...
# graph by beta and N_Tx (Figure 6)
...
terr_plot1
ggsave(paste(f4, "Fialkowski_Figure6.tiff", sep = ""), dpi = 800, terr_plot1, 
...
# 3 tx groups
terr1 <- read.table(paste(f2, "tx3_terr_betas_1.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr2 <- read.table(paste(f2, "tx3_terr_betas_2.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr3 <- read.table(paste(f2, "tx3_terr_betas_3.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
...
# graph by beta and N_Tx (Figure 7)
...
Fig7 <- grid_arrange_shared_legend(terr_plot1, terr_plot2)
ggsave(paste(f4, "Fialkowski_Figure7.png", sep = ""), dpi = 800, Fig7, 
...
# 5 tx groups
terr1 <- read.table(paste(f2, "tx5_terr_betas_1.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr2 <- read.table(paste(f2, "tx5_terr_betas_2.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
terr3 <- read.table(paste(f2, "tx5_terr_betas_3.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
...
# graph by beta and N_Tx (Figure 8)
sum_terr1 <- summarySE(terr, measurevar = "TERR1", groupvars = c("beta", "N_Tx", "Model"))
sum_terr2 <- summarySE(terr, measurevar = "TERR2", groupvars = c("beta", "N_Tx", "Model"))
...
Fig8 <- grid_arrange_shared_legend(rmse_plot1, rmse_plot2, rmse_plot3, rmse_plot4)

ggsave(paste(f4, "Fialkowski_Figure8.png", sep = ""), dpi = 800, Fig8, 
...
rmse1 <- read.table(paste(f2, "rmse_betas_1.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
rmse2 <- read.table(paste(f2, "rmse_betas_2.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
rmse3 <- read.table(paste(f2, "rmse_betas_3.txt", sep = ""), header = TRUE, sep = "\t", 
                   quote = "")
...
sum_rmse <- summarySE(rmse, measurevar = "MSE", groupvars = c("beta", "Tx", "N_Tx", "Model"))
...
# MSE graph by Tx and N_Tx (Figure 9)
sum_rmse <- summarySE(rmse, measurevar = "MSE", groupvars = c("beta", "Tx", "N_Tx", "Model"))
sum_rmse <- sum_rmse[order(sum_rmse$beta, sum_rmse$Tx, sum_rmse$N_Tx, sum_rmse$Model), ]
...
mse_plot1 <- ggplot(sum_rmse[sum_rmse$beta == 1, ], 
  ...
  xlab("") + 
  scale_y_continuous("Small Tx Effect", breaks = seq(0.23, 0.35, 0.01), 
                     limits = c(0.23, 0.351)) +
  scale_colour_manual(name = "", values = color, 
                      labels = c("EN25  ", "EN50  ", "EN75  ", "Lasso  ", 
                                 "BLasso  ", "MARS  ")) + #l = 40)
...
mse_plot2 <- ggplot(sum_rmse[sum_rmse$beta == 2, ], 
  ...
  facet_wrap(~Tx, labeller = tx_labeller) + theme_bw() + 
  xlab("") + 
  scale_y_continuous("Medium Tx Effect", breaks = seq(0.23, 0.35, 0.01), 
                     limits = c(0.23, 0.351)) +
  scale_colour_manual(name = "", values = color, 
                      labels = c("EN25  ", "EN50  ", "EN75  ", "Lasso  ", 
                                 "BLasso  ", "MARS  ")) + #l = 40)
...
mse_plot3 <- ggplot(sum_rmse[sum_rmse$beta == 3, ], 
  ...
  facet_wrap(~Tx, labeller = tx_labeller) + theme_bw() + 
  xlab("Sample Size per Tx Group") + 
  scale_y_continuous("Large Tx Effect", breaks = seq(0.23, 0.35, 0.01), 
                     limits = c(0.23, 0.351)) +
  scale_colour_manual(name = "", values = color, 
                      labels = c("EN25  ", "EN50  ", "EN75  ", "Lasso  ", 
                                 "BLasso  ", "MARS  ")) + #l = 40)
...
Fig9 <- grid_arrange_shared_legend(mse_plot1, mse_plot2, mse_plot3)

ggsave(paste(f4, "Fialkowski_Figure9.tiff", sep = ""), dpi = 800, Fig9, 
       width = 6.7, height = 8.9, units = "in")

# R^2 table by Tx and N_Tx
sum_rmse <- summarySE(rmse, measurevar = "R2", groupvars = c("beta", "Tx", "N_Tx", "Model"))
sum_rmse <- sum_rmse[order(sum_rmse$beta, sum_rmse$Tx, sum_rmse$N_Tx, sum_rmse$Model), ]
...
# MSE: training sets
rmse1 <- read.table(paste(f2, "train_rmse_betas_1.txt", sep = ""), header = TRUE, sep = "\t", 
                    quote = "")
rmse2 <- read.table(paste(f2, "train_rmse_betas_2.txt", sep = ""), header = TRUE, sep = "\t", 
                    quote = "")
rmse3 <- read.table(paste(f2, "train_rmse_betas_3.txt", sep = ""), header = TRUE, sep = "\t", 
                    quote = "")
...
# Percentile CI
# beta_1
pr_2 <- read.table(paste(f2, "perc_pr_2_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "perc_pr_3_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "perc_pr_5_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
# beta_2
pr_2 <- read.table(paste(f2, "perc_pr_2_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "perc_pr_3_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "perc_pr_5_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
# beta_3
pr_2 <- read.table(paste(f2, "perc_pr_2_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "perc_pr_3_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "perc_pr_5_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
Fig10 <- grid_arrange_shared_legend(t_plot1, t_plot2, t_plot3)

ggsave(paste(f4, "Fialkowski_Figure10.png", sep = ""), dpi = 800, Fig10, 
       width = 6.7, height = 8.7, units = "in")

# Normal CI
# beta_1
pr_2 <- read.table(paste(f2, "pr_2_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "pr_3_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "pr_5_betas_1.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
# beta_2
pr_2 <- read.table(paste(f2, "pr_2_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "pr_3_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "pr_5_betas_2.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
# beta_3
pr_2 <- read.table(paste(f2, "pr_2_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_3 <- read.table(paste(f2, "pr_3_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
pr_5 <- read.table(paste(f2, "pr_5_betas_3.txt", sep = ""), header = TRUE, sep = "\t", quote = "")
...
# Graphs of actual vs. predicted response values or studentized residuals vs. predicted response values 
# in validation sets for small tx effects, n = 100 per tx group

f3 <- "Logistic_Errors/Beta1/"
...
par(mfrow = c(3, 6), cex = 1.5, 
    mai = c(1.22,1.22,0.52,0.42))

for (i in c(1, 4, 7)) {
  Vars2 <- readRDS(paste(f3, "Vars2_", tx[i], "_", n_tx[i], "_betas_1.rds", sep = ""))
  for (r in 1:3) {
    for (j in 2:ncol(Vars2[[r]])) {
      if (r == 1 & j == 2) {
        plot(Vars2[[r]][, j], Vars2[[r]][, 1], xlab = "Predicted", 
             ylab = paste("Actual (r = ", ind[r], ")", sep = ""), main = colnames(Vars2[[r]])[j]) 
      } else if (r == 1) {
        plot(Vars2[[r]][, j], Vars2[[r]][, 1], xlab = "Predicted", 
             ylab = "Actual", main = colnames(Vars2[[r]])[j]) 
      } else if (j == 2) {
        plot(Vars2[[r]][, j], Vars2[[r]][, 1], xlab = "Predicted", 
             ylab = paste("Actual (r = ", ind[r], ")", sep = "")) 
      } else {
        plot(Vars2[[r]][, j], Vars2[[r]][, 1], xlab = "Predicted", 
             ylab = "Actual")
      }
  ...
  for (r in 1:3) {
    for (j in 2:ncol(Vars2[[r]])) {
      if (r == 1 & j == 2) {
        plot(Vars2[[r]][, j], (Vars2[[r]][, 1] - Vars2[[r]][, j])/sd(Vars2[[r]][, 1]), xlab = "Predicted", 
             ylab = paste("Std Residuals (r = ", ind[r], ")", sep = ""), main = colnames(Vars2[[r]])[j]) 
      } else if (r == 1) {
        plot(Vars2[[r]][, j], (Vars2[[r]][, 1] - Vars2[[r]][, j])/sd(Vars2[[r]][, 1]), xlab = "Predicted", 
             ylab = "Std Residuals", main = colnames(Vars2[[r]])[j]) 
      } else if (j == 2) {
        plot(Vars2[[r]][, j], (Vars2[[r]][, 1] - Vars2[[r]][, j])/sd(Vars2[[r]][, 1]), xlab = "Predicted", 
             ylab = paste("Std Residuals (r = ", ind[r], ")", sep = "")) 
      } else {
        plot(Vars2[[r]][, j], (Vars2[[r]][, 1] - Vars2[[r]][, j])/sd(Vars2[[r]][, 1]), xlab = "Predicted", 
             ylab = "Std Residuals")
      }
...
# histograms of bootstrapped beta coefficients
ind <- c(1, 500, 1000, 1500, 2000)
...
for (i in 1:length(tx)) {
  EN25 <- readRDS(paste(f3, "EN1_boot_", tx[i], "_", n_tx[i], "_a25_", 1, ".rds", sep = ""))
  EN50 <- readRDS(paste(f3, "EN1_boot_", tx[i], "_", n_tx[i], "_a50_", 1, ".rds", sep = ""))
  EN75 <- readRDS(paste(f3, "EN1_boot_", tx[i], "_", n_tx[i], "_a75_", 1, ".rds", sep = ""))
  Lasso <- readRDS(paste(f3, "EN1_boot_", tx[i], "_", n_tx[i], "_", 1, ".rds", sep = ""))
  Betas <- list()
  Betas[[1]] <- Betas[[2]] <- Betas[[3]] <- Betas[[4]] <- Betas[[5]] <- list()
  Betas[[1]][[1]] <- EN25[[1]]$coefs
  Betas[[1]][[2]] <- EN50[[1]]$coefs
  Betas[[1]][[3]] <- EN75[[1]]$coefs
  Betas[[1]][[4]] <- Lasso[[1]]$coefs
  Betas[[2]][[1]] <- EN25[[500]]$coefs
  Betas[[2]][[2]] <- EN50[[500]]$coefs
  Betas[[2]][[3]] <- EN75[[500]]$coefs
  Betas[[2]][[4]] <- Lasso[[500]]$coefs
  Betas[[3]][[1]] <- EN25[[1000]]$coefs
  Betas[[3]][[2]] <- EN50[[1000]]$coefs
  Betas[[3]][[3]] <- EN75[[1000]]$coefs
  Betas[[3]][[4]] <- Lasso[[1000]]$coefs
  Betas[[4]][[1]] <- EN25[[1500]]$coefs
  Betas[[4]][[2]] <- EN50[[1500]]$coefs
  Betas[[4]][[3]] <- EN75[[1500]]$coefs
  Betas[[4]][[4]] <- Lasso[[1500]]$coefs
  Betas[[5]][[1]] <- EN25[[2000]]$coefs
  Betas[[5]][[2]] <- EN50[[2000]]$coefs
  Betas[[5]][[3]] <- EN75[[2000]]$coefs
  Betas[[5]][[4]] <- Lasso[[2000]]$coefs
  saveRDS(Betas, paste(f2, "Betas_", tx[i], "_", n_tx[i], "_betas_1.rds", sep = ""))
}

models <- c("EN25", "EN50", "EN75", "Lasso")

par(mfrow = c(5, 4), cex = 1, cex.lab = 1.8, cex.main = 1.8, cex.axis = 1.7, 
    mai = c(1.12,1.0,0.42,0.32))

Betas <- readRDS(paste(f2, "Betas_2_100_betas_1.rds", sep = ""))

for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["Tx", ], freq = F, breaks = 20,
           xlab = expression(hat(beta)), xlim = c(0, 1.5),
           ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["Tx", ], freq = F, xlab = expression(hat(beta)), 
           xlim = c(0, 1.5),ylab = "Density", breaks = 20,main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["Tx", ], freq = F, xlab = expression(hat(beta)), 
           xlim = c(0, 1.5),ylab = "Density", breaks = 20,main = "")
    } else {
      hist(Betas[[r]][[j]]["Tx", ], freq = F, xlab = expression(hat(beta)), 
           xlim = c(0, 1.5),ylab = "Density", breaks = 20,main = "")
    }
...
for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["C1M5", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 2.5), ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["C1M5", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 2.5), ylab = "Density", main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["C1M5", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 2.5), ylab = "Density", main = "")
    } else {
      hist(Betas[[r]][[j]]["C1M5", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 2.5), ylab = "Density", main = "")
    }
...
Betas <- readRDS(paste(f2, "Betas_3_100_betas_1.rds", sep = ""))
for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["Tx_1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 1), ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["Tx_1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 1), ylab = "Density", main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["Tx_1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 1), ylab = "Density", main = "")
    } else {
      hist(Betas[[r]][[j]]["Tx_1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0, 1), ylab = "Density", main = "")
    }
...
for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["Tx_2", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.7, 0),ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["Tx_2", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.7, 0),ylab = "Density", main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["Tx_2", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.7, 0),ylab = "Density", main = "")
    } else {
      hist(Betas[[r]][[j]]["Tx_2", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.7, 0),ylab = "Density", main = "")
    }
...
Betas <- readRDS(paste(f2, "Betas_5_100_betas_1.rds", sep = ""))
for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["C3M1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.4, 0.3),ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["C3M1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.4, 0.3),ylab = "Density", main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["C3M1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.4, 0.3),ylab = "Density", main = "")
    } else {
      hist(Betas[[r]][[j]]["C3M1", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(-0.4, 0.3),ylab = "Density", main = "")
    }
...
for (r in 1:5) {
  for (j in 1:4) {
    if (r == 1 & j == 1) {
      hist(Betas[[r]][[j]]["Age", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0.3, 0.8),ylab = "Density", main = models[j])
    } else if (r == 1) {
      hist(Betas[[r]][[j]]["Age", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0.3, 0.8),ylab = "Density", main = models[j])
    } else if (j == 1) {
      hist(Betas[[r]][[j]]["Age", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0.3, 0.8),ylab = "Density", main = "")
    } else {
      hist(Betas[[r]][[j]]["Age", ], freq = F, xlab = expression(hat(beta)), 
           breaks = 20, xlim = c(0.3, 0.8),ylab = "Density", main = "")
    }
